...
export class NotesService {
  ...
  static getStats() {
    ...
    stats = stats.filter((obj: any) => obj.active || obj.archived);

    return stats;
  }

  static update(updateNoteDto: UpdateNoteDto, id: number) {
    const note = {
      id: id,
    ...
    const oldNote = notes.find((obj: any) => obj.id === note.id);
    if (!oldNote) return false;

    notes = notes.filter((obj: any) => obj.id !== oldNote.id);
  ...
  static addFromArchive(note: any) {
    notes.push(note);
  }
}
